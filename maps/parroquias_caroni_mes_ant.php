<?php
	...
	FROM `sucesos` AS s
	WHERE s.municipio_id = $muni_id AND year(fecha_suceso) =$ano AND MONTH(fecha_suceso)=MONTH(DATE_ADD(CURDATE(),INTERVAL -1 MONTH))
	...
	$query_fechas = $db->Prepare("SELECT $ano AS ano, MONTH(DATE_ADD(CURDATE(),INTERVAL -1 MONTH)) AS mes");
	...
	$acu_mes_parr_univer = "unare: " .$rs_homici_mes_parr_univer->Fields('acu_mes_parr_univer');
	...
	$query_sucesos = $db->Prepare("SELECT COUNT(fecha_suceso) AS tot_homi_mes, $ano AS ano, MONTH(DATE_ADD(CURDATE(),INTERVAL -1 MONTH)) AS mes,
		MONTH(DATE_ADD(CURDATE(),INTERVAL -1 MONTH)) AS mes_ant
		FROM sucesos AS s
		WHERE YEAR(fecha_suceso) = $ano AND MONTH(fecha_suceso) = MONTH(DATE_ADD(CURDATE(),INTERVAL -1 MONTH)) AND municipio_id = $muni_id AND delito_detalle_id = $delito_deta");
...
<html>
<head>
	<title>Homicidios por Parroquias Municipio <?php echo $muni_descri;?> Mes anterior, Estado <?php echo $muni_descri;?>, Venezuela</title>
...
<body>
	<div class="panel panel-primary">
		<div class="panel-heading">Mapa Distribucion Acumulada de homicidios por Parroquias,
			Municipio <?php echo $muni_descri;?>, Estado <?php echo $muni_descri;?>, A&ntilde;o <?php echo $ano;?>, Mes <?php echo $mes_ant;?> Total = <a href="../sucesos/lis_homicidios_con_ano_mes_caro.php?ano=<?php echo $ano;?>&mes=<?php echo $mes_ant;?>" class = "btn-warning" target="blank"><?php echo $tot_homi_car_mes_ant;?></a>
	...
	var unare = "<?php echo $acu_mes_parr_univer; ?>" ;
	...
	[pozoverde, 8.259059870475873, -62.62533187866211, 6, '../sucesos/lis_homicidios_con_ano_mes_caro_parro.php?ano=<?php echo $ano?>&mes=<?php echo $mes?>&parro=<?php echo $parro_pozo?>'],
	  ...
	  [unare, 8.27732165765, -62.72884368896, 3, '../sucesos/lis_homicidios_con_ano_mes_caro_parro.php?ano=<?php echo $ano?>&mes=<?php echo $mes?>&parro=<?php echo $parro_uni?>'],
