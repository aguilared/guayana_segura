<?php
	...
	$query_homici_mes = $db->Prepare("SELECT count(*) AS acu_mes
		FROM `sucesos`
		WHERE `municipio_id` = 3 AND year(fecha_suceso) =year(now()) AND MONTH(fecha_suceso)=Month(now()) AND delito_detalle_id = 7");
	
	$query_homici_mes_ant = $db->Prepare("SELECT count(*) AS acu_mes_ant
		FROM `sucesos`
		WHERE `municipio_id` = 3 AND year(fecha_suceso) =year(now()) AND MONTH(fecha_suceso)=(Month(now())-1) AND delito_detalle_id = 7");
	
	$query_homici_ano = $db->Prepare("SELECT count(*) AS acu_ano
		FROM `sucesos`
		WHERE `municipio_id` = 3 AND year(fecha_suceso) =year(now()) AND delito_detalle_id = 7");
	...
	$query_homici_mes_sf = $db->Prepare("SELECT count(*) AS acu_mes_sf
		...
		AND delito_detalle_id = 7 AND capital_sector = 'sf'");
	//puerto Ordaz
	$query_homici_mes_poz = $db->Prepare("SELECT count(*) AS acu_mes_poz
		FROM `sucesos` AS s
		INNER JOIN parroquias AS p ON s.parroquia_id = p.parroquia_id
		WHERE s.municipio_id = 3 AND year(fecha_suceso) =year(now()) AND MONTH(fecha_suceso)=Month(now()) 
		AND delito_detalle_id = 7 AND capital_sector = 'poz'");
	//San felix
	$query_homici_mes_ant_sf = $db->Prepare("SELECT count(*) AS acu_mes_ant_sf
		FROM `sucesos` AS s
		INNER JOIN parroquias AS p ON s.parroquia_id = p.parroquia_id
		WHERE s.municipio_id = 3 AND year(fecha_suceso) =year(now()) AND MONTH(fecha_suceso)=(Month(now())-1) 
		AND delito_detalle_id = 7 AND capital_sector = 'sf'");
	//puerto Ordaz
...
<html lang="en">
	<head>
		<title>Guayana Segura. Administracion de Sucesos</title>
		<meta charset="utf-8">
		...
		<div class="row">
			<div class="col-sm-12">
				<div class="panel panel-primary">
					<div class="panel-heading">Guayana Segura. Administracion de Sucesos</div>
							...
							<?php
								...
								while(!$rs_sucesos->EOF){
									echo '<tr>';
										...
										echo '<td width=250>';
										echo '<a class="btn update btn-success" href="frm_modi_suceso.php?suceso_id='.$rs_sucesos->Fields('suceso_id').'">Modificar</a>';
										echo '&nbsp;';
										echo '<a class="btn btn-danger" id="delete" href="trata_borra_suceso.php?suceso_id='.$rs_sucesos->Fields('suceso_id').'">Borrar</a>';
										echo '&nbsp;';
										echo '</td>';
									echo '</tr>';
									$rs_sucesos->MoveNext();
