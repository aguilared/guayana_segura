<?php
	...
	$delito_id = 1;
	$delito_detalle_id = 7;  //defect homicidio
	$estado_id = 7;			//Defect Bolivar
	$municipio_id = 3;		//Defect Caroni
	$movil_id = 1; // defect movil
	$arma_id = 1; // defect arma
...
<div class="modal fade" id="myModal">
					...
					<div class="panel-body">
						...
						<form class="form-horizontal" role="form" id="contactform">
							<div class="form-group">
								...
								<div class="col-xs-3">
									<input class="form-control input-sm" name="hora" id="hora" type="text" value="00:00">
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-xs-10">
									<select class="form-control input-sm" name="delito_detalle_id" id="delito_detalle_id" onchange="carga_arma($('#delito_detalle_id').val());return false;">
							...
							<div class="form-group">
								...
								<div class="col-xs-4">
									<input class="form-control input-sm" name="edad" id="edad" value="0" type="text">
								</div>
							</div>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			...
			$("#contactform").validate({
				...
				submitHandler: function(form){
					...
					var dataString = 'fecha='+$('#fecha').val()+'&hora='+$('#hora').val()+'&delito_id='+$('#delito_id').val()+'&delito_detalle_id='+$("select#delito_detalle_id").val()+'&titulo='+$('#titulo').val()+'&fuente='+$('#fuente').val()+'&otra_fuente1='+$('#otra_fuente1').val()+'&otra_fuente2='+$('#otra_fuente2').val()+'&municipio='+$("select#municipio").val()+'&parroquia='+$("select#parroquia").val()+'&nombre_victima='+$('#nombre_victima').val()+'&sexo='+$("select#sexo").val()+'&edad='+$('#edad').val()+'&profesion='+$("select#profesion").val()+'&tipo_arma='+$("select#tipo_arma").val()+'&movil_id='+$("select#movil_id").val()+'&sector='+$('#sector').val()+'&resena='+$('#resena').val()+'&latitud='+$('#latitud').val()+'&longitud='+$('#longitud').val()+'&usuario='+$('#usuario').val();
					//alert(dataString);
					$.ajax({
						type: "POST",
		...
		//cargando el select arma
		function carga_arma(delito_detalle_id){
			//alert('llego'+delito_detalle_id);
			var parametros = {
			"delito_detalle_id" : delito_detalle_id
			};
			$.ajax({
				data: parametros,
				url: 'carga_arma.php',
				...
				beforeSend: function () {
					$("#tipo_arma").empty();
					$("#tipo_arma").html("Procesando, espere por favor...");
				},
				success: function (response) {
				$("#tipo_arma").html(response);
				}
			});
		}
